...
let remindersController = {
  ...
  tags:(req,res)=>{
    ...
    if(inputvalue== "add"){
      ...
      database[name].reminders[0].tag.push(req.body.tag)
      let Show=true
      res.render("reminder/single-reminder", { reminderItem: searchResult,
                                                Show:Show })
    }else{
        let tag = database[name].reminders[0].tag[inputvalue]
        let result = database[name].reminders[0].tag.filter(elem => elem !== tag)
        database[name].reminders[0].tag=result
        res.redirect("/reminder/" + reminderToFind)
    }
